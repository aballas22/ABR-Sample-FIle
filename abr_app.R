...
ui <- fluidPage(theme = shinytheme("darkly"),
                navbarPage(
                  ...
                  tabPanel("Home",
                           sidebarPanel(
                             h6("something:"),
                             tags$img(src="sima.gif",height=50,width=50),
                             ...
                             selectInput("select", label = h4("Would you like to change the Jewett Labels?"), 
                             ...
                             selectInput("select2", label = h4("Would you like to change the Jewett Valleys?"), 
                                         choices = list("No" = FALSE, "Yes" = TRUE), 
                                         selected = FALSE),
                             shinyjs::hidden(
                               div(
                           ...
                           mainPanel(
                             tabsetPanel(
                               tabPanel("ABR Plot - Peaks",
                                        
                                        plotOutput("plot1")),
                               tabPanel("ABR Plot - Valleys",
...
server <- function(input, output, session) {
  ...
  output$plot1 <- renderPlot({
    ...
    assign("test1",final_plot,envir = .GlobalEnv)
  })
  ...
  output$plot2 <- renderPlot({
    ...
    latency_analysis <- analyze(Jewett,avg_list,"Measurements")
    analysis_data[nrow(analysis_data) + 1,] <- latency_analysis
    
    area_analysis[nrow(area_analysis) + 1,] <- c("Measurements",areas)
